...
const gameStoreSub = new Rx.Subject();
...
export function playStart() {
  const betAmout = getDataByKey('bet');
...
export function getBetScope(value, minus) {
  const s = [0, 100, 1000, 10000, 100000, 500000, Infinity];
  const b = [10, 100, 1000, 10000, 100000];
  const i = minus ? s.findIndex(v => v >= value) : s.findIndex(v => v > value);
...
export function getDefaultBet() {
  ...
  if (userid) {
    ...
    const gameBase = getDataByKey('gameBase'),
      gameScore = gameBase.get('gameScore'),
      TCoin = gameBase.get('TCoin');
    const b = Math.max(10, Math.min(500000, Math.max(gameScore, TCoin) * 0.01)),
      s = getBetScope(b);
    betAmount = b - (b % s);
...
export function getMaxBet() {
  const gameBase = getDataByKey('gameBase');
  const gameScore = gameBase.get('gameScore'),
    TCoin = gameBase.get('TCoin');
...
export function getGameBaseInfo() {
  ...
  axios.get(`?act=game_gamebase&st=queryUserAccount&gameId=${gameId}&Type=1`, { timeout: 5000 }).then(response => {
